        ...
        <div className="space-y-8">
          {posts.map((post) => (
            <article key={post.slug} className="border-b border-border/50 pb-8 last:border-b-0 group">
              ...
              <p className="text-sm text-muted-foreground mb-4 opacity-80">{post.date}</p>
              ...
              >
                Read more
                <ArrowUpRight className="w-4 h-4 transition-transform duration-300 ease-in-out group-hover:translate-x-1 group-hover:-translate-y-1" />
              </Link>
            </article>
          ))}
        </div>
